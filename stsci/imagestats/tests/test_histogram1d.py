...
def test_histogram(gaussian_image):
    ...
    h = histogram1d(arrayInput=data, nbins=nbins, binWidth=binwidth, zeroValue=minv)
...
def test_histogram_upper_bin_flt_roundoff(gaussian_image):
    ...
    h = histogram1d(arrayInput=data, nbins=87, binWidth=0.01637466996908188, zeroValue=minv)
    assert np.sum(h.histogram) == np.sum(np.logical_and(data >= minv, data < 1.0441159009933472))
