# Contributing to stsci.imagestats

`stsci.imagestats` is an open source package written in Python.
The source code is available at https://github.com/spacetelescope/stsci.imagestats.
...
[open an issue](https://github.com/spacetelescope/stsci.imagestats/issues).
...
1. [Fork this repository](https://github.com/spacetelescope/stsci.imagestats/fork).
2. Clone your fork to your local machine:

   ```shell
   git clone https://github.com/YOUR_USERNAME/stsci.imagestats
   cd stsci.imagestats/
   ```

3. Add the `upstream` repository, as a remote, to your local clone:
   ```shell
   git remote add upstream https://github.com/spacetelescope/stsci.imagestats
   ```
...
5. [Install `stsci.imagestats` to your development environment.](#creating-a-development-environment)
...
8. [Open a new Pull Request](https://github.com/spacetelescope/stsci.imagestats/pulls) requesting that your changes be merged into the `main` branch of this repository.
...
As `stsci.imagestats` is constantly evolving, you will often encounter the situation where you've made changes to your branch, but in that time there are new commits on `upstream/main` from other developers.
...
When developing `stsci.imagestats` (or any other Python package), you should install the package locally to a development environment.
...
```shell
mamba create -n stsci.imagestats_dev_env python=3.13
mamba activate stsci.imagestats_dev_env
pip install -e .
hx .
```
...
1. Create a new empty environment called `stsci.imagestats_dev_env`:
   ```shell
   mamba create -n stsci.imagestats_dev_env python=3.13
   ```
2. "Activate" the environment (change shell variables in the current session to point to the isolated Python installation):
   ...
   mamba activate stsci.imagestats_dev_env
   ```
3. Install the local package (`stsci.imagestats`) to your environment in "editable mode", so that any code changes will be instantly reflected in the installed package (useful for testing):
   ```shell
   pip install -e .
   ```
...
## Making simultaneous changes to `stsci.imagestats` and one of its dependencies

If you need to make a change in `stsci.imagestats` that requires a simultaneous change to one of its dependencies, also install that dependency from your local machine [to your development environment](#creating-a-development-environment).
